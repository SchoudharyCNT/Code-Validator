...
const SuggestRuleInputSchema = z.object({
  ...
  category: z.string().describe('The category of the rule (e.g., Security, Performance).'),
  subcategory: z.string().optional().describe('The subcategory of the rule (optional).'),
  codeExample: z.string().describe('An example of code that violates the rule.'),
  description: z.string().describe('A description of the desired behavior or coding practice.'),
});
...
const SuggestRuleOutputSchema = z.object({
  ...
  description: z.string().describe('The suggested detailed description for the rule.'),
  severity: z.enum(['Warning', 'Error', 'Info']).describe('The severity level of the rule.'),
});
